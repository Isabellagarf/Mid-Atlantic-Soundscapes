...
Deployment_data <- read.csv("code/csv/Deployment_metadata_mid-Atl.csv")




# option to set order of site names 
Site_order <- as.character(unique(Deployment_data$SITE_NAME))

# order factor levels
Status_order <- c("Processed Sucessfully","ERROR", "Omitted")

# Number of factors to include in coloring of timeline 
levels(factor(Deployment_data$DEPLOYMENT_STATUS)) # 9
