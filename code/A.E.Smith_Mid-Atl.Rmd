...
```
#only plotting sunset hours- with chorus presence
```{r}
#DB01
...
allSite_chorusPresence <- allSite_chorusPresence %>%
  mutate(UTC_date = date(UTC))

runDetectionExplorer(allSite_chorusPresence)

#this won't plot for some reason- "need cols 'UTC', typecol, must have columns UTC, species"

plotAcousticScene(x=allSite_chorusPresence, by='site', combineYears=FALSE, freqMap=data.frame(species=c('X1000_chorus', 'X500_chorus', 'cusk_chorus', 'oyster_chorus'), freqMin=c(140, 60, 1200, 150), freqMax=c(6500, 930, 2600, 3650)))

```
#inital diel patterens for each chorus between sites- weekly medians
```{r}
...
ggplot(DB01500MedQuants, aes(x= UTC_hour, y= med_500, group= 1))+
  ...
  xlab("Hour (UTC)")+
  ylab(expression(paste('Median 500Hz Octave Level SPL (db re 1 \u00b5Pa)')))+
  ggtitle("DB01 unknown pulse- peak week median")+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))

#now add in diel times (sunset, sunrise, twilight etc etc) and do this for other choruses & sites
