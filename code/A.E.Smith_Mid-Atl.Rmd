...
DB01_500OL <- DB01_OLtemps_daily %>%
  group_by(UTC, med_temp) %>%
  dplyr::summarize(Med_OL = median(med_500, na.rm= TRUE))
DB01_500OL$OL_band <- "OL_500"
DB01_500OL$PLATFORM <- "DB01"
DB01_1000OL <- DB01_OLtemps_daily %>%
  group_by(UTC, med_temp) %>%
  dplyr::summarize(Med_OL = median(med_1000, na.rm= TRUE))
DB01_1000OL$OL_band <- "OL_1000"
DB01_1000OL$PLATFORM <- "DB01"
DB01_2000OL <- DB01_OLtemps_daily %>%
  group_by(UTC, med_temp) %>%
  dplyr::summarize(Med_OL = median(med_2000, na.rm= TRUE))
DB01_2000OL$OL_band <- "OL_2000"
DB01_2000OL$PLATFORM <- "DB01"
DB01_daily_OL <- DB01_500OL %>%
...
OL_500_dailyTemps_Plot = ggplot(data = DB01_OLtemps_daily, mapping = aes(x =UTC)) + 
  ...
  scale_y_continuous("Daily Median 500 Octave Level (UNITS)", 
                     sec.axis = sec_axis(~ . -90, name = "Temperature (°C)")
  )+
  theme_bw()+
...
OL_1000_dailyTemps_Plot = ggplot(data = DB01_OLtemps_daily, mapping = aes(x =UTC)) + 
  ...
  scale_y_continuous("Daily Median 1000 Octave Level (UNITS)", 
                     sec.axis = sec_axis(~ . -90, name = "Temperature (°C)")
  )+
  theme_bw()+
  scale_x_date(limits= as.Date(c("2022-07-07", "2024-11-03")))
...
OL_2000_dailyTemps_Plot = ggplot(data = DB01_OLtemps_daily, mapping = aes(x =UTC)) + 
  ...
  scale_y_continuous("Daily Median 2000 Octave Level (UNITS)", 
                     sec.axis = sec_axis(~ . -90, name = "Temperature (°C)")
  )+
  theme_bw()+
  scale_x_date(limits= as.Date(c("2022-07-07", "2024-11-03")))
