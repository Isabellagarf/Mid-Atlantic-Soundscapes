...
ESOL_500_dailyTemps_Plot = ggplot(data = ES01_OLtemps_daily, mapping = aes(x =UTC)) + 
  ...
  geom_hline(yintercept = 100, col= "black")+
  theme_bw()+
  scale_x_date(limits= as.Date(c("2023-10-13", "2024-09-29")))+
  labs(title= "ES01",
       x= "Date (UTC)")
...
boxplot(Internal_temp_C ~ site, data = all_site_temps, xlab= "Site", ylab= "Temp")
...
```{r}
#two-way anova
  #does SPL vary based on OL and time of day?

ES01_diel_shapiro <- ES01_suntimes %>%
  group_by(sun_time) %>%
  summarise(p.value= shapiro.test(OL_2000)$p.value)
print(ES01_diel_shapiro) #failed normality (p-values less than 0.05)
leveneTest(ES01_suntimes$OL_2000, ES01_suntimes$sun_time) #failed equal variances (p-value less than 0.001)

kruskal.test(OL_2000 ~ sun_time, data = ES01_suntimes)
boxplot(OL_500 ~ sun_time, data = ES01_suntimes, xlab= "Time of day", ylab= "OL SPL")
...
#Do SPL vary based on site and temperature?

all_OLTemps <- DB01_OLTemps %>%
  full_join(ES01_OLTemps)%>%
  full_join(CB01_OLTemps)


all_OLTemps <- all_OLTemps %>%
  drop_na(PLATFORM)

cor.test(DB01_OLTemps$OL_1000, DB01_OLTemps$Internal_temp_C, method= "kendall")

ggplot(all_OLTemps, aes(x = Internal_temp_C, y = OL_1000, group= PLATFORM, color= PLATFORM))+
  geom_point() +
