...
melted_all_TOLSiteSeason$TOL_band = factor(melted_all_TOLSiteSeason$TOL_band,
                                        levels= c("med_25","med_31.5","med_40","med_50", "med_63","med_80","med_100", "med_125","med_160","med_200", "med_250","med_315","med_400", "med_500","med_630","med_800", "med_1000","med_1250","med_1600", "med_2000","med_2500","med_3150", "med_4000","med_5000","med_6300", "med_8000","med_10000","med_12500", "med_16000", "med_20000"),
...
plotPSD(x=all_sites, style='quantile', q=0.01, dbRange= c(30, 120),facet='PLATFORM', ncol = 3)+
#  facet_grid(factor('PLATFORM', levels = c('DB01', 'DB02', 'DB03', 'ES01', 'ES02', 'ES03', 'CB01', 'CB02', 'CB03')))+
  theme_bw()+
  labs(y= (expression(paste("Power Spectral Density dB re 1 \u00b5Pa" ^ "2", "/Hz"))))
...
#DB01_tempsPlot= ggplot(DB01_OLTemps, aes(x = Datetime_UTC, y = Internal_temp_C))+
  geom_line()+
  theme_bw()+
  labs(x= "Date-time (UTC)", y= "Temperature (°C)")

#DB01_500Plot= ggplot(DB01_OLTemps, aes(Datetime_UTC, OL_500))+
  geom_line()+
  theme_bw()+
  labs(x= "Date-time (UTC)", y= (expression(paste("Median 500 Hz Octave Band SPL (dB re \u00b5Pa" ^ "2", "/Hz)"))))

#DB01_tempsPlot/DB01_500Plot +plot_layout(axes= "collect", guides= "collect")
...
#DB01_OLtemps_daily <- DB01_OLTemps %>% 
  group_by(UTC) %>% 
  dplyr::summarize(med_temp = median(Internal_temp_C, na.rm=TRUE))

#DB01_500OL <- DB01_OLTemps %>%
  group_by(UTC) %>%
  dplyr::summarize(Med_OL = median(OL_500, na.rm= TRUE))
#DB01_500OL$OL_band <- "OL_500"
#DB01_500OL$PLATFORM <- "DB01"
#DB01_1000OL <- DB01_OLTemps %>%
  group_by(UTC) %>%
  dplyr::summarize(Med_OL = median(OL_1000, na.rm= TRUE))
#DB01_1000OL$OL_band <- "OL_1000"
#DB01_1000OL$PLATFORM <- "DB01"
#DB01_2000OL <- DB01_OLTemps %>%
  group_by(UTC) %>%
  dplyr::summarize(Med_OL = median(OL_2000, na.rm= TRUE))
#DB01_2000OL$OL_band <- "OL_2000"
#DB01_2000OL$PLATFORM <- "DB01"
#DB01_OLtemps_daily <- DB01_OLtemps_daily %>%
  full_join(DB01_500OL) %>%
  full_join(DB01_1000OL)%>%
  full_join(DB01_2000OL)
...
mlrmodel <- lm(med_2000 ~ med_temp+site, data = all_OLTemps)
summary(mlrmodel)
...
ES01_202404_minOL <- read.csv(here("data/ES01_202404_minOL.csv"))
ES01_202404_minOL <- ES01_202404_minOL[-c(1)]
ES01_202404_minOL$UTC <- as.POSIXct(ES01_202404_minOL$UTC, format= "%Y-%m-%dT%H:%M:%SZ")

CB01_202206_minOL <- read.csv(here("data/CB01_202206_minOL.csv"))
CB01_202206_minOL <- CB01_202206_minOL[-c(1)]
CB01_202206_minOL$UTC <- as.POSIXct(CB01_202206_minOL$UTC, format= "%Y-%m-%dT%H:%M:%SZ")

```
#minute-level sunset OL
```{r}
...
DB01_OLtemps_daily$X500_chorus = factor(DB01_OLtemps_daily$X500_chorus,
                                        levels= c("no", "yes"),
                                        labels= c("Absent", "Present"))
ES01_OLtemps_daily <- ES01_OLtemps_daily %>%
  full_join(ES01_chorusPresence)
ES01_OLtemps_daily$X500_chorus = factor(ES01_OLtemps_daily$X500_chorus,
                                        levels= c("no", "yes"),
                                        labels= c("Absent", "Present"))
CB01_OLtemps_daily <- CB01_OLtemps_daily %>%
  full_join(CB01_chorusPresence)
CB01_OLtemps_daily$X500_chorus = factor(CB01_OLtemps_daily$X500_chorus,
                                        levels= c("no", "yes"),
                                        labels= c("Absent", "Present"))
all_temp_presence <- DB01_OLtemps_daily %>%
  full_join(ES01_OLtemps_daily) %>%
  full_join(CB01_OLtemps_daily)
...
ggplot(all_temp_presence, aes(x = med_temp, y = med_500, group= X500_chorus, color= X500_chorus))+
  geom_point() +
  geom_smooth(method='glm', level= 0.90)+
  stat_poly_eq(formula = y ~ x, 
               aes(label = paste(..eq.label.., ..rr.label.., ..p.value.label.., sep = "*`,`~")), 
  ...
  xlab("Median daily temperature (°C))")+
  ylab(expression(paste('Median 500 Hz Octave Level SPL (dB re 1 \u00b5Pa)')))+
  scale_color_manual(labels= c("Absent", "Present"), values= c("#DB2207", "#4B8320"))+
  scale_fill_manual(values= c("#DB2207", "#4B8320"))+
  theme_bw()+
  guides(color= guide_legend(title= "Chrous Presence"))+
  facet_wrap(~site)


#repeat with no site differences and with each chorus
```