...
ES01_HMD$month <- month(ES01_HMD$UTC)
CB01_HMD$month <- month(CB01_HMD$UTC)

DB02_HMD$month <- month(DB02_HMD$UTC)
ES02_HMD$month <- month(ES02_HMD$UTC)
CB02_HMD$month <- month(CB02_HMD$UTC)
CB03_HMD$month <- month(CB03_HMD$UTC)

DB03_HMD$month <- month(DB03_HMD$UTC)
ES03_HMD$month <- month(ES03_HMD$UTC)
...
all_OLSiteSeason<- all_OLSites %>% 
  ...
  dplyr::summarize(med_63 = median(OL_63, na.rm=TRUE),med_125 = median(OL_125, na.rm=TRUE), med_500 = median(OL_500, na.rm=TRUE), med_1000 = median(OL_1000, na.rm=TRUE),med_2000= median(OL_2000, na.rm= TRUE), med_8000= median(OL_8000, na.rm= TRUE))

ggplot(DB_OLSiteSeason, aes(x= season, y = med_500, color= PLATFORM, group= PLATFORM))+
  geom_point()+
  theme_bw()
...
OL_500_dailyTemps_Plot = ggplot(data = DB01_OLtemps_daily, mapping = aes(x =UTC)) + 
...
OL_1000_dailyTemps_Plot = ggplot(data = DB01_OLtemps_daily, mapping = aes(x =UTC)) + 
  geom_line(mapping = aes(y = med_temp+90), color="pink", size=1)+
...
OL_2000_dailyTemps_Plot = ggplot(data = DB01_OLtemps_daily, mapping = aes(x =UTC)) + 
  geom_line(mapping = aes(y = med_temp+90), color="pink", size=1)+
...
OL_500_dailyTemps_Plot/OL_1000_dailyTemps_Plot/OL_2000_dailyTemps_Plot+ plot_layout(axis_titles = "collect", guides= "collect")
...
DB01_envData_daily$Moon.Phase = case_when(DB01_envData_daily$Moon.Phase == "Full Moon" ~ 1,
                                          DB01_envData_daily$Moon.Phase == "New Moon" ~ 2,
                                          DB01_envData_daily$Moon.Phase == "First Quarter" ~ 0,
                                          DB01_envData_daily$Moon.Phase == "Last Quarter" ~ 0,
