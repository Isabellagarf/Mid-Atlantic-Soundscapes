...
## OCTAVE LEVEL FILES
```{r}
DB01_OL <- read.csv(here("data/OL/DB01_OL.csv"))
DB01_OL <- DB01_OL[-c(1)]
DB01_OL$UTC <- as.POSIXct(DB01_OL$UTC, format= "%m/%d/%Y %H", tz = "GMT")
DB02_OL <- read.csv(here("data/OL/DB02_OL.csv"))
DB02_OL$UTC <- as.POSIXct(DB02_OL$UTC, format= "%Y-%m-%dT%H:%M:%SZ", tz = "GMT")
DB03_OL <- read.csv(here("data/OL/DB03_OL_missing2022.csv"))
DB03_OL$UTC <- as.POSIXct(DB03_OL$UTC, format= "%Y-%m-%dT%H:%M:%SZ", tz = "GMT")
ES01_OL <- read.csv(here("data/OL/ES01_OL.csv"))
ES01_OL <- ES01_OL[-c(1)]
ES02_OL <- read.csv(here("data/OL/ES02_OL.csv"))
ES02_OL <- ES02_OL[-c(1)]
ES03_OL <- read.csv(here("data/OL/ES03_OL.csv"))
ES03_OL <- ES03_OL[-c(1)]
CB01_OL <- read.csv(here("data/OL/CB01_OL.csv"))
CB01_OL <- CB01_OL[-c(1)]
CB02_OL <- read.csv(here("data/OL/CB02_OL.csv"))
CB02_OL <- CB02_OL[-c(1)]
CB03_OL <- read.csv(here("data/OL/CB03_OL.csv"))
CB03_OL <- CB03_OL[-c(1)]


runSoundscapeExplorer(DB01_OL)
```
